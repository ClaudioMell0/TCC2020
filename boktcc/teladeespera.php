...
session_start();
 ...
 //echo $_POST['senha'];
 $_SESSION['senha_battle'] = $_POST['senha'];
 //echo  $_SESSION['senha_battle'];
 $ID_SENHABATTLE =$_SESSION['senha_battle'];
  //conexão
  ...
  $sql3 = "SELECT * FROM `tb_usuario` CROSS JOIN `tb_battleroyale_usuario` where id_usuario = id_usuario2 and id_battleroyale_id = " . $_POST['senha'] ."";
  $result3 = mysqli_query ($conecta, $sql3);
  $posicao = 0;

//seleciona as informações do usuario para mostrar na estrutura de repetição
  $query = "select id_usuario,st_nome,st_curso,st_ano, st_nivel from tb_usuario
  group by id_usuario
  desc;
  ";
 ...
 //Muda o fuso para o horario de brasilia
  ...
  $sql = "select * from tb_battleroyale
            where id_battleroyale = " . $_POST['senha'] . " and dia_inicio >='" . date("Y-m-d") . "' and hora_inicio > '" . date("H:i:s") . "'" ;

            $result = mysqli_query ($conecta, $sql);
            if(mysqli_num_rows($result) == 0){
              header("Location: codigobattle.php?id_erro=1");
            }else{
              $row = mysqli_fetch_assoc($result);
            //  Insere o usuario na tabela, pra ele participar do battle royale;
            $Inserir_batlle = "INSERT INTO tb_battleroyale_usuario (id_battleroyale,id_usuario,st_acertos_battleroyale,flag_ativo,id_pergunta_errada)
            VALUES ('".$ID_SENHABATTLE."','".$id."','0','1','0')";
          $INSERIR= mysqli_query($conecta,$Inserir_batlle); 
...
//calculo para a hora ficar no formato certo
  ...
  if (distance < 0) {
    ...
    setTimeout( function() {
 window.location.href = "pergBattle.php";
