...
if(!$conecta){
    ...
    $sqli = "select * from tb_prova_perguntas";
    $resultt = mysqli_query ($conecta, $sqli);
...
if ($_SESSION['id_provaperg']==""){
    $_SESSION['id_provaperg']=1;
}
//$_SESSION['id_battleperg'];
$_SESSION['id_provaperg']=$_SESSION['id_provaperg']+1;
$id_provaperg = $_SESSION['id_provaperg'];
$provaperg = "SELECT * FROM tb_prova_perguntas WHERE id_prova = 1 and id_perguntas_x = '$id_provaperg'";

$retorni = mysqli_query($conecta,$provaperg);
    $rowl  = mysqli_fetch_assoc($retorni);
    $_SESSION['id_pperg'] = $rowl ["id_perguntas"];
    $id_provaa = $_SESSION['id_pperg'];
...
if($_SESSION['perguntas_sorteadas'] == ""){
    $query = "SELECT * FROM tb_perguntas WHERE id_perguntas = '$id_perguntas_prova'";
}else{
    $query = "SELECT * FROM tb_perguntas WHERE id_perguntas = '$id_perguntas_prova' and id_perguntas ";
}
...
//Leva o usuário para outra página caso a resposta for errada
        header("Location: finalprova.php");
    }
...
if($_SESSION['perguntas_sorteadas'] == ""){
    ...
    $query = "SELECT * FROM tb_perguntas WHERE st_materia = 'biologia' ORDER BY RAND() limit 1";
}else{
    $query = "SELECT * FROM tb_perguntas WHERE st_materia = 'biologia' and id_perguntas not in (" .  $_SESSION['perguntas_sorteadas'] . ") ORDER BY RAND() limit 1";
}
...
<form name="form1" method="POST" action="Pergprova.php" style="margin-top:10%; margin-left:-25%;" id="form">
